...
export default function PostMatchSummary({ finalData, onGoHome }) {
  const { t1, t2, setupData } = finalData;
  ...
  const calculateResult = () => {
    if (t2.runs > t1.runs) {
      const t2TotalPlayers = Object.keys(t2.stats.batting).length;
      const wicketsRemaining = (t2TotalPlayers - 1) - t2.wickets;
      return `${t2.team} Won by ${Math.max(1, wicketsRemaining)} wickets!`;
    } else if (t1.runs > t2.runs) {
      return `${t1.team} Won by ${t1.runs - t2.runs} runs!`;
    } else {
      return "Match Tied!";
    }
  };
  const actualResultMessage = calculateResult();
  ...
  // --- UPGRADED MULTI-PAGE PDF GENERATOR ---
  const downloadPDF = async () => {
    ...
    try {
      ...
      // Keep adding pages as long as there is image content left to draw
      while (heightLeft > 0) {
        position = heightLeft - totalPdfHeight; // Shift the image up
        pdf.addPage();
        pdf.addImage(dataUrl, 'PNG', 0, position, pdfWidth, totalPdfHeight);
        heightLeft -= pdfPageHeight;
      }
      ...
      alert("Failed to download PDF. Please ensure the app is fully loaded.");
    } finally {
      setIsDownloading(false);
    }
  };

  // --- MOBILE UI SCORECARD RENDERER ---
  const renderMobileScorecard = (teamData) => (
    ...
    <>
      ...
      <div className="flex flex-col h-full bg-slate-950 text-white select-none overflow-hidden relative z-10">
        <div className="bg-gradient-to-b from-emerald-900/40 to-slate-950 pt-8 pb-4 px-4 border-b border-slate-800 shrink-0 text-center">
          ...
          <h2 className="text-md font-black text-amber-500 uppercase tracking-widest mb-2 drop-shadow-md">{actualResultMessage}</h2>
      ...
      {/* --- REDESIGNED BEAUTIFUL PDF TEMPLATE (HIDDEN FROM UI, FULLY RENDERED) --- */}
      <div style={{ position: 'absolute', top: 0, left: '-9999px', zIndex: -9999 }}>
        <div ref={pdfRef} className="w-[850px] bg-[#020617] text-white p-12 font-sans flex flex-col gap-8 border-[8px] border-slate-900 rounded-3xl m-4">
          ...
          <div className="flex justify-between items-end border-b-2 border-slate-800 pb-6">
            ...
            <div className="text-right">
              <p className="text-slate-400 font-bold uppercase tracking-widest text-sm flex items-center justify-end gap-2 mb-1">
          ...
          <div className="bg-emerald-950/30 border border-emerald-500/30 rounded-2xl p-8 text-center relative overflow-hidden">
            ...
            <p className="text-2xl font-bold text-slate-300 uppercase tracking-widest mt-2 relative z-10">{actualResultMessage}</p>
          ...
          <div className="grid grid-cols-3 gap-6">
            {motm && (
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl">
                ...
                <p className="text-slate-400 text-sm font-mono bg-slate-950 inline-block px-3 py-1.5 rounded-lg">
                  {motm.runs > 0 && `${motm.runs} Runs `} {motm.wickets > 0 && `• ${motm.wickets} Wkts`}
                </p>
              </div>
            )}
            {bestBatter && (
          ...
          {[t1, t2].map((teamData, idx) => (
            <div key={idx} className="bg-slate-900 rounded-2xl border border-slate-800 p-6">
              ...
              <table className="w-full text-left mb-8 border-collapse">
                <thead>
                  <tr className="bg-slate-950 text-slate-500 text-sm uppercase tracking-widest">
                    <th className="py-3 px-4 rounded-l-lg">Batter</th>
                    ...
                    <th className="py-3 px-4 text-center rounded-r-lg">SR</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.keys(teamData.stats.batting).map(p => {
                    ...
                    return (
                      <tr key={p} className="border-b border-slate-800/50 last:border-0">
                        <td className="py-4 px-4">
                          <span className={`font-bold text-lg ${s.out ? 'text-slate-300' : 'text-emerald-400'}`}>{p} {s.out ? '' : '*'}</span>
                          {s.out && <div className="text-xs text-slate-500 italic mt-1">{s.dismissal}</div>}
                        </td>
                        <td className="py-4 px-4 text-center font-bold text-lg text-white">{s.runs}</td>
                        <td className="py-4 px-4 text-center text-slate-400">{s.balls}</td>
                        <td className="py-4 px-4 text-center text-slate-400">{s.fours}</td>
                        <td className="py-4 px-4 text-center text-slate-400">{s.sixes}</td>
                        <td className="py-4 px-4 text-center text-slate-500 font-mono">{sr}</td>
                      </tr>
                    );
                  })}
                </tbody>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-950 text-slate-500 text-sm uppercase tracking-widest">
                    <th className="py-3 px-4 rounded-l-lg">Bowler</th>
                    <th className="py-3 px-4 text-center">O</th>
                    <th className="py-3 px-4 text-center">M</th>
                    <th className="py-3 px-4 text-center">R</th>
                    <th className="py-3 px-4 text-center">W</th>
                    <th className="py-3 px-4 text-center rounded-r-lg">Econ</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.keys(teamData.stats.bowling).map(p => {
                    ...
                    return (
                      <tr key={p} className="border-b border-slate-800/50 last:border-0">
                        <td className="py-4 px-4 font-bold text-lg text-amber-400">{p}</td>
                        <td className="py-4 px-4 text-center text-slate-400 font-mono">{overs}</td>
                        <td className="py-4 px-4 text-center text-slate-500">0</td>
                        <td className="py-4 px-4 text-center text-white text-lg">{s.runs}</td>
                        <td className="py-4 px-4 text-center font-bold text-white text-lg">{s.wickets}</td>
                        <td className="py-4 px-4 text-center text-slate-500 font-mono">{econ}</td>
                      </tr>
                    );
                  })}
                </tbody>
